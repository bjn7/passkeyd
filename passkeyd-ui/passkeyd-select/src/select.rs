...
use passkeyd_abi::{component::title_bar_component, database::layout::OtherUI, theme};
...
impl SelectionUI {
    pub fn view(&self) -> Element<'_, UserResponse, theme::StylisedTheme> {
        let title_bar = title_bar_component(
            passkeyd_locale::translate!("ui.select.select.title_bar"),
            UserResponse::Deny,
        );

        let description = text(passkeyd_locale::translate!("ui.select.select.description",
        ...
        let users_component = self
            ...
            .enumerate()
            .map(|(index, other_ui)| {
                passkeyd_abi::component::user_component(
                    &self.rp,
                    other_ui,
        ...
        // let users = passkeyd_abi::get_select_component(&self.rp, &self.other_uis, None);
